...
- State: local store hooks in `store/`
...
- Requires: `.env.local` with `OPENAI_API_KEY` for `/api/generate`
...
- Prefer explicit types for public data shapes in `lib/types.ts`.
- Use `type` aliases for object shapes (no `interface` in this codebase so far).
- Keep functions small and pure in `lib/` helpers.
...
- App Router pages live in `app/` and default export the page component.
- Route handlers in `app/api/*/route.ts` use `export async function GET/POST`.
- Client components include `"use client"` at the top.
- Use `next/font` for font loading (see `app/layout.tsx`).

State & Storage
- State hooks live in `store/`.
- Storage helpers live in `lib/storage.ts`.
- Update types in `lib/types.ts` when data model changes.
...
Styling Conventions
- Use Tailwind classes and existing design tokens from `app/globals.css`.
- For reusable patterns, create components in `components/`.
...
- Service worker registration is in `components/ServiceWorkerRegister.tsx`.
...
- Update translations in `lib/translations.ts` if UI text changes.
